...
本次插圖實驗除了調整 JPEG quality，也同時將 PNG 轉換為 JPEG，因此檔案大小同時受到「格式轉換」與「quality」影響。

```text
//cat-illustration 的 console 結果
=============
cat-illustration
...
注意：插圖節省 92%、96%、98%，不全是因為 quality 降低，也有很大部分來自：
```
PNG → JPEG
```
的格式轉換。

#### 補充：PNG 是無損壓縮（lossless）
